...
for (i = 0; i < textArray.length; i++) {
    ...
    if (textArray[i].slice(0, 1) == "[") {
        ...
        let wordsArray = textArray[i].split(" ")
        ...
        for (j = pileArray[from].length - amountOfCrates; j < pileArray[from].length; j++) {
            pileArray[end].push(pileArray[from][j])
            if (j == pileArray[from].length - 1) pileArray[from].splice(pileArray[from].length - amountOfCrates, amountOfCrates)
        }
